...
//Integration
void ext_bk_mp(double **tr,double **tr2, double **tr3, double **tr4, double *log_km, double *log_pkm, double *cosm_par, double redshift, int fit_full,int kp_dim,  
int num_tr, int num_tr2,int num_tr3,int num_tr4, double *bk_mipj_ar)
{
    ...
    //monopole
#ifdef OMP
    #pragma omp parallel for private(i, val, err)
#endif
    for (i = 0 ; i < num_tr; i ++)
    {
        double ka= tr[i][0];
...
void _ext_bk_mp(double **tr,double **tr2, double **tr3, double **tr4, double *log_km, double *log_pkm, double *cosm_par, double redshift, int fit_full,int kp_dim,  
    int num_tr, int num_tr2,int num_tr3,int num_tr4, double *bk_mipj_ar)
    {
        ...
        #pragma omp parallel for private(i, val, err)
        for(i = 0; i < num_tr + num_tr2 + num_tr3 + num_tr4; i++){
            ...
            else if (i >= num_tr && i < num_tr + num_tr2){
            ...
            else if (i >= num_tr + num_tr2 && i < num_tr + num_tr2 + num_tr3){
                i -= (num_tr + num_tr2);
                double ka= tr3[i][0];
                double kb= tr3[i][1];
                double kc= tr3[i][2];
            ...
            else if (i >= num_tr + num_tr2 + num_tr3 && i < num_tr + num_tr2 + num_tr3 + num_tr4){
                i -= (num_tr + num_tr2 + num_tr3);
                double ka= tr4[i][0];
                double kb= tr4[i][1];
                double kc= tr4[i][2];
