...
print(cosm_par[0])
cosmo_jax = jc.Cosmology(Omega_c=0.1200 / h2, Omega_b=0.02237 / h2, h=h, sigma8 = 0.81, n_s=0.9649,
Omega_k=0., w0=-1., wa=0.)
...
_ax = ax.ravel()
k_ev2 = k_ev**2 # Fixing undefined variable typo from original script
_ax[0].plot(k_ev, k_ev2 * bk_sugi['000'])
_ax[0].plot(k_ev, k_ev2 * bk_sugi['202'])
...
#EFT parameters
alpha0, alpha2, alpha4 = 0,0,0    # PEFT(k) = (alpha0 + alpha2mu^2+ alpha4mu^4)  k^2 Plin(k)
...
k_ev_C2 = k_ev_C**2 # Fixing undefined variable typo from original script
_ax[5].plot(k_ev_C, k_ev_C2 * B000_C, label = 'Ref')
_ax[5].plot(k_ev_C, k_ev_C2 * bk_sugi['000'], label = "GEO")
